...
public class ViewFeedback extends javax.swing.JFrame {
    ...
    private void jShowActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jShowActionPerformed
        String str;
        ArrayList<String> feedbacklines = new ArrayList<String>();
        
        ViewFeedback viewfeedback = new ViewFeedback();
        
        try{
            FileReader payment = new FileReader("/Users/yongpua/Desktop/Java/Java Assignment/src/apu/cafeteria/Review.txt");
            BufferedReader payment2 = new BufferedReader(payment);
            
            try{
                while((str = payment2.readLine()) != null){
                    feedbacklines.add(str);
                }
            } catch(IOException e){
                
            }
         }catch (FileNotFoundException e){
             JOptionPane.showMessageDialog(viewfeedback, "File not found. Please try again.");
         }
        String[] line = new String[feedbacklines.size()];
        line = feedbacklines.toArray(line);
        
        for(int i = 0; i< feedbacklines.size();i++){
            String showfeedback = line[i];
            jFeedback.setText(jFeedback.getText() + "\n" + showfeedback);
        }
